...
\vspace{-5pt}
{\bf Post-doc Researcher at Stanford University}\hfill\textbf{2016 - Now}\\
...
\begin{my_item}
\item Efficient algorithm for parsing natural language into logical forms.
\item 11x-16x faster and 13\% more accurate than state-of-the-art.
\end{my_item}
\end{my_item}
...
\begin{my_item}
\item New algorithms for non-convex optimization (with applications to deep learning).
\item Improved over traditional optimization methods in various of metrics.
\end{my_item}
\item Convex optimization [\ref{stochastic-jmlr},\ref{stochastic-icml15}].
\begin{my_item}
\item New algorithm for generic convex optimization
\item Orders-of-magnitude speed-up over SGD in theory and in practice.
\end{my_item}
\item Crowdsourcing [\ref{spectral-jmlr},\ref{spectral-nips14}].
\begin{my_item}
\item Estimating true labels from noisy crowdsourced datasets.
\item The first algorithm that achieves the best possible statistical accuracy.
\end{my_item}
\item Personalized recommender systems [\ref{taxonomy-wsdm14}].
\begin{my_item}
\item New algorithm for improving the quality of online-shopping recommendation.
\end{my_item}
\item Click modeling for web search [\ref{understanding-wsdm12},\ref{user-kdd11},\ref{characterize-www11},\ref{learning-cikm10},\ref{explore-cikm10},\ref{incorporating-sigir10}]
\begin{my_item}
\item Understanding users' feedback by modeling their search and browsing behaviors.
...
\begin{my_item}
\item Communication-efficient distributed algorithms that work on hundreds or more machines.
\item Guarantee the best possible statistical accuracy and the least possible computation time.
\end{my_item}
\item Foundamental theory of distributed computing [\ref{lower-colt14},\ref{information-nips13},\ref{optimality}].
\begin{my_item}
\item Understanding the foundamental trade-offs between communication, computation and statistical accuracy for any distributed system.
\end{my_item}

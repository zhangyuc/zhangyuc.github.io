...
\vspace{-10pt}
\begin{my_item}
\item Build the next-generation dialogue system platform for all Microsoft products. Our system is built on a novel program synthesis framework that supports arbitrarily complex natural language understanding with seamless multi-turn, multi-domain state tracking. The bot is able to handles long-tail queries by learning from data with minimal developer intervention. Serving MS Office and 3rd-party parteners with AI assiatant and customer service bots (\href{https://v.youku.com/v_show/id_XNDE3MTExODQ0MA}{demo link}).
\item I lead the system's core representation and modeling projects. I am the main designer and the lead developer of the program synthesis framework, and a major contributor to the system's capability of compositional semantic parsing, state tracking, error handling and natural language generation.

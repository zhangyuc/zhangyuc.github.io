...
\begin{my_item}
\item New efficient algorithm for parsing natural language into logical forms.
...
\item Stronger optimality guarantees; improved empirical performance on DNN/CNN/LSTM.
\end{my_item}
...
\begin{my_item}
\item Algorithm for estimating true labels from noisy crowdsourced data.
\item Optimality guarantee: first algorithm to achieve the best possible statistical rate.
\end{my_item}
\item Personalized recommender systems [\ref{taxonomy-wsdm14}].
\begin{my_item}
\item Non-parametric language model for characterizing very sparse user-item relations.
...
\begin{my_item}
\item Data mining from the massive search click log.
...
\begin{my_item}
\item Communication-efficient algorithms for distributed machine learning on big clusters.
...
\begin{my_item}
\item Understanding the foundamental trade-offs between communication, computation and statistical accuracy for an arbitrary distributed system.
\end{my_item}
